...
class scene3 extends Phaser.Scene{
	constructor(){
		super('getAcquaintedK');
	}
	preload(){
		this.load.image('bg2', 'assets/background2.png')
		this.load.image('keyboard2','assets/keyboard2.png');
		this.load.image('lefthand2','assets/left_hand2.png');
		this.load.image('righthand','assets/right_hand.png');
		this.load.image('beefinger', 'assets/bee.png')

		this.load.image('line','assets/line.png');
		this.load.image('s1','assets/keys/k1.png');
		this.load.image('s2','assets/keys/k2.png');
		this.load.image('s3','assets/keys/k3.png');
	...
	create(){
		...
		this.add.image(765, 513, 'beefinger');
		
		var key = [];
		var x = 250;
		var y = 100;
		for (var i = 0; i < 8; i++) {
			key[i] = this.add.image(x, y, 's1'); x += 100;
		}

		var index = 0;
		var current_key = this.add.image(key[0].x, key[0].y, 's2');
		var current_line = this.add.image(key[0].x, key[0].y + 60, 'line');

		this.input.keyboard.on('keydown', function (event) {
			if (event.keyCode === Phaser.Input.Keyboard.KeyCodes.K)
            {
				this.add.image(key[index].x, key[index].y, 's3');
				...
				if (index == 8)
					...
					current_key = this.add.image(key[index].x, key[index].y, 's2');

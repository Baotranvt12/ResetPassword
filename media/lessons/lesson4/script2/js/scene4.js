...
class scene4 extends Phaser.Scene{
	constructor(){
		super('getAcquaintedL');
	}
	preload(){
		this.load.image('bg2', 'assets/background2.png')
		this.load.image('keyboard3','assets/keyboard3.png');
		this.load.image('lefthand3','assets/left_hand3.png');
		this.load.image('righthand','assets/right_hand.png');
		this.load.image('beefinger', 'assets/bee.png')

		this.load.image('line','assets/line.png');
		this.load.image('d1','assets/keys/l1.png');
		this.load.image('d2','assets/keys/l2.png');
		this.load.image('d3','assets/keys/l3.png');
	...
	create(){
		...
		var y = 100;
		for (var i = 0; i < 8; i++) {
			key[i] = this.add.image(x, y, 'd1'); x += 100;
		}

		var index = 0;
		var current_key = this.add.image(key[0].x, key[0].y, 'd2');
		var current_line = this.add.image(key[0].x, key[0].y + 60, 'line');

		this.input.keyboard.on('keydown', function (event) {
			if (event.keyCode === Phaser.Input.Keyboard.KeyCodes.L)
            {
				this.add.image(key[index].x, key[index].y, 'd3');
				...
				if (index == 8)
					...
					current_key = this.add.image(key[index].x, key[index].y, 'd2');
					current_line = this.add.image(key[index].x, key[index].y + 60, 'line');
				}

            }
			else
				this.scene.start('getAcquaintedL'); 
		}, this);
